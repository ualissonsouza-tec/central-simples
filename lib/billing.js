...
async function ensureTrialForUser(user) {
  ...
  const startedAt = parseSqlDate(user.trial_started_at);
  const currentEndsAt = parseSqlDate(user.trial_ends_at);

  if (startedAt && currentEndsAt) {
    const targetEndsAt = addDays(startedAt, trialDays);
    const isTrial = !user.subscription_status || user.subscription_status === 'trialing';

    if (isTrial && currentEndsAt < targetEndsAt) {
      await dbRun(
        `UPDATE users
         SET trial_ends_at = ?, updated_at = datetime('now')
         WHERE id = ?`,
        [toIsoSql(targetEndsAt), user.id]
      );

      return {
        ...user,
        trial_ends_at: toIsoSql(targetEndsAt),
      };
    }
