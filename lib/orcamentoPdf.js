...
function resolveCompanyLogo(company) {
  const dataLogo = decodeLogoDataUrl(company?.logo_data_url);
  if (dataLogo) return dataLogo;

  const logoPath = company?.logo_path ? path.join(__dirname, '..', company.logo_path) : null;
  return logoPath && fs.existsSync(logoPath) ? logoPath : null;
}
...
function drawCompanyHeader(doc, company, options) {
  ...
  const logoSource = resolveCompanyLogo(company);
  const cx = 96;
  const cy = 56;
  const radius = 34;
  ...
  if (logoSource) {
    ...
    doc.image(logoSource, cx - radius, cy - radius, { width: radius * 2, height: radius * 2 });
    doc.restore();
